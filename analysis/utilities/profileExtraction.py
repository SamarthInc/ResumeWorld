...
from dataExtractor.scripts.textCleaner import TextCleaner
...
class DataExtractor:
    ...
    def extract_links(self):
        ...
        links = re.findall(link_pattern, self.clean_text)
        return links
    ...
    def extract_links_extended(self):
        ...
        """
        link_pattern = r'((http(s)?:\/\/)?([\w]+\.)?linkedin\.com\/(pub|in|profile)\/([-a-zA-Z0-9]+)\/*)'
        links = re.search(link_pattern, self.clean_text).group()
        return links.strip()
    ...
    def extract_phone_numbers(self):
        ...
        """
        phone_number_pattern = r'(((\+\d{1,2}\s?)?1?\-?\.?\s?\(?\d{3}\)?[\s.-]?)?\d{3}[\s.-]?\d{4})'
        phone_numbers = re.search(phone_number_pattern, self.text).group()
        return (phone_numbers).strip()
